...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
  ...
  if (room == "🍓쁘 띠 공 지 방🍓") {
    ...
    if (msg.indexOf("/도움말") == 0) {
      replier.reply(
        "● 도움말\n☆원정 신청, 취소방법\n★/[N][닉네임][신청]을 입력\nEx) /N 닉네임 신청\nEx) /N 닉네임 취소\n-N:1~5\n(1:하드, 2:노말, 3:하드2, 6: 하드3, 4:반레온, 5:길레 )\n-신청시 키알 조심\n-한번에 한명만 신청 가능\n(별)취소시 신청 닉네임과 동일해야됨!\n-반레온, 길레는 상시 모집"
      );
      replier.reply(
    ...
    if (msg.indexOf("/변경") == 0 && msg.indexOf("-") != -1) {
      ...
      if (
        ...
        if (Gdif === lv1) {
          ...
          lv6 = msg.substring(msg.indexOf("-") + 1, msg.length);
          lv6 = lv6.replace(/(^\s*)|(\s*$)/, "");
        ...
        t3 = t3.replace(/(^\s*)|(\s*$)/, "");
        replier.reply(call_list(t3));
      } else if (msg.indexOf(t6) != -1 && t6.toString().length == Gdif.length) {
        t6 = msg.substring(msg.indexOf("-") + 1, msg.length);
        t6 = t6.replace(/(^\s*)|(\s*$)/, "");
        replier.reply(call_list(t6));
      }
    }
    ...
    if (msg.indexOf("/") == 0 && msg.indexOf("신청") != -1) {
      if (msgDatat4.length < 10 && msg.indexOf("/4") != -1) {
        ...
        (swit == false && msg.indexOf("/2") != -1) ||
        (swit == false && msg.indexOf("/3") != -1)
      ) {
