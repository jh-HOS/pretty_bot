...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
  ...
  if (room == "🍓쁘 띠 공 지 방🍓") {
    ...
    if (msg.indexOf("/도움말") == 0) {
      replier.reply(
        "● 도움말\n☆원정 신청, 취소방법\n★/[N][닉네임][신청]을 입력\nEx) /N 닉네임 신청\nEx) /N 닉네임 취소\n-N:1~6\n(1:노말 아카이럼, 2:카힐1, 3:매그너스, 4:매그너스2, 5:카힐2, 6:하드 아카이럼 )\n-신청시 키알 조심\n-한번에 한명만 신청 가능\n(별)취소시 신청 닉네임과 동일해야됨!\n- 길레는 상시 모집"
      );
      replier.reply(
        "○ 명령어\n/도움말\n/목록 : 여제 목록 보기\n/길레 : 길레 목록 보기\n파티목록보기 : /1파티 (/파티번호+파티)"
      );
    ...
    if (swit == true && msg.indexOf("/리셋") != -1) {
      if (msg.indexOf("1") != -1) {
        ...
        t5 = 2210;
        lv5 = "하아카";
        replier.reply("5팟 리셋완료!");
      } else if (msg.indexOf("6") != -1) {
        msgDatat6 = [];
        msgDatat6_list = [];
        t6 = 2220;
        lv6 = "카힐2";
        replier.reply("6팟 리셋완료!");
      } else if (msg.indexOf("all") != -1) {
        msg_origin("all");
        swit = false;
        replier.reply("보스 명단 리셋완료!");
      }
    }
    ...
    if (swit == true && msg.indexOf("/") == 0 && msg.indexOf("신청") != -1) {
      if (msgDatat1.length < 10 && msg.indexOf("/1") != -1) {
        ...
        return;
      } else if (msgDatat5.length < 10 && msg.indexOf("/5") != -1) {
        msgDatat5.push(msg_filter(msg));
        replier.reply(call_list(t5, "+"));
      } else if (msgDatat5.length >= 10 && msg.indexOf("/5") != -1) {
        replier.reply("5팟" + lv5 + " 마감입니다!");
        return;
      } else if (msgDatat6.length < 10 && msg.indexOf("/6") != -1) {
        msgDatat6.push(msg_filter(msg));
        replier.reply(call_list(t6, "+"));
      } else if (msgDatat6.length >= 10 && msg.indexOf("/6") != -1) {
        replier.reply("6팟" + lv6 + " 마감입니다!");
        return;
      }
    }
    ...
    if (msg.indexOf("/") == 0 && msg.indexOf("취소") != -1) {
      ...
      } else if (swit == true && msg.indexOf("/5") != -1) {
      ...
      } else if (swit == true && msg.indexOf("/6") != -1) {
    ...
    if (swit == true && msg.indexOf("/5파티") == 0) {
      replier.reply(answer2(t5));
    }
    if (swit == true && msg.indexOf("/6파티") == 0) {
